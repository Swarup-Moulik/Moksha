...
/** @brief Replaces all uses of `oldVal` with `newVal` in the given function. */
    ...
    for (auto *user : allocUses) {
      if (auto *gep = llvm::dyn_cast_or_null<GetElementPtrInst>(user)) {
        if (gep->getIndices().size() == 2) {
          auto *baseIdx =
              llvm::dyn_cast_or_null<ConstantInt>(gep->getIndices()[0]);
          auto *fieldIdx =
              llvm::dyn_cast_or_null<ConstantInt>(gep->getIndices()[1]);
